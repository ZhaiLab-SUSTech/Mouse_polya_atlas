...
def plot_final_heatmap(
    ...
    submatrix_df = pd.read_csv(submatrix_path, index_col=0)
    modules_df = pd.read_csv(modules_path)
    
    gene_cat = modules_df['Module'].tolist()
    cat_order = modules_df['Module'].unique()
    
    colors = sns.color_palette("husl", n_colors=len(cat_order))
    hex_colors = [mcolors.rgb2hex(color) for color in colors]
    
    h = ma.Heatmap(
        submatrix_df.values, cmap="viridis", width=6, height=6
    )
    
    h.hsplit(labels=gene_cat, order=cat_order, spacing=0.005)
    h.vsplit(labels=gene_cat, order=cat_order, spacing=0.005)
    
    h.add_left(mp.Chunk(cat_order, colors=hex_colors, fontsize=8), pad=0.05)
    h.add_top(mp.Chunk([""]*len(cat_order), colors=hex_colors), pad=0.05) # No labels on top
...
def main():
    ...
    submatrix_csv_path = os.path.join(python_output_dir, "clustered_polya_submatrix.csv")
    
    r_script_path = "scripts/Downstream_analysis/cutree_polya.R"
    r_output_dir = "results/downstream_analysis/05_polya_modules"
    modules_csv_path = os.path.join(r_output_dir, "polya_modules.csv")
    final_heatmap_path = os.path.join(r_output_dir, "final_polya_annotated_heatmap.pdf")
    ...
    high_polya_idx = [gene_to_order_map[g] for g in high_expression_genes if g in gene_to_order_map]

    if len(high_polya_idx) < 2: return
        
    sub_matrix = distance_matrix[np.ix_(high_polya_idx, high_polya_idx)]
    ...
    reordered_symbols = [isoform2symbol.get(g, '') for g in reordered_isoforms]
    final_index_names = [f"{sym}-{iso}" for sym, iso in zip(reordered_symbols, reordered_isoforms)]
    
    cophenet_sub_df = pd.DataFrame(reordered_matrix, index=final_index_names, columns=final_index_names)
    os.makedirs(python_output_dir, exist_ok=True)
    cophenet_sub_df.to_csv(submatrix_csv_path)
    print(f"Clustered sub-matrix saved to {submatrix_csv_path}")
    ...
    if r_success and os.path.exists(modules_csv_path):
        ...
        print("Skipping final heatmap generation due to errors in the R script execution.", file=sys.stderr)
