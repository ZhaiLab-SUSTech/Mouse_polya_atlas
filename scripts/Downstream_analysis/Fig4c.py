...
def main():
    ...
    module_file_path = "results/downstream_analysis/06_expression_modules/expression_modules.csv"
    output_dir = "results/downstream_analysis/10_go_analysis_expression"
    
    gmt_file_path = "data/annotation/msigdb_v2024.1.Mm_GMTs/m5.all.v2024.1.Mm.symbols.gmt"
    
    os.makedirs(output_dir, exist_ok=True)

    print(f"Loading expression modules from {module_file_path}...")
    ...
    final_df.to_csv(os.path.join(output_dir, "go_enrichment_results_expression.csv"), index=False)
    print(f"Final GO results table saved in {output_dir}")

    plot_go_dotplot(
        plot_df=final_df,
        output_path=os.path.join(output_dir, "go_enrichment_dotplot_expression.pdf"),
        title="GO Enrichment for Expression Modules"
    )
